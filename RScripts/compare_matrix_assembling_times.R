...
# Simulation
times = NULL
for (sqrt_nnodes_pow in seq(from = 2, by = 0.2, length.out = 3))
{
  ...
  for(rep in 1:10)
  {
    ...
    time_exec = system.time(R_stiff(FEMbasis))
    times = rbind(times,data.frame(rep = rep, sqrt_nnodes = sqrt_nnodes, nnodes = sqrt_nnodes^2, t_user = time_exec[1], t_system = time_exec[2], t_elapsed = time_exec[3], Code = "R Stiff"))
    
    time_exec = system.time(CPP_get.FEM.Stiff.Matrix(FEMbasis))
    times = rbind(times,data.frame(rep = rep, sqrt_nnodes = sqrt_nnodes, nnodes = sqrt_nnodes^2, t_user = time_exec[1], t_system = time_exec[2], t_elapsed = time_exec[3], Code = "C++ Stiff"))
    
    
    time_exec = system.time(CPP_get.FEM.PDE.Matrix(FEMbasis, PDE_parameters_anys))
    ...
    time_exec = system.time(CPP_get.FEM.PDE.sv.Matrix(FEMbasis, PDE_parameters_sv))
