...
void MixedFERegression<InputHandler,Integrator,ORDER>::smoothLaplace()
{
	...
	typedef EOExpr<Real, StiffXX<Integrator, ORDER> > ETStiffXX;
	typedef EOExpr<Real, StiffYY<Integrator, ORDER> > ETStiffYY;

	Mass<Integrator, ORDER> EMass;
	StiffXX<Integrator, ORDER> EStiffXX;
	StiffYY<Integrator, ORDER> EStiffYY;

	ETMass mass(EMass);
	ETStiffXX stiffXX(EStiffXX);
	ETStiffYY stiffYY(EStiffYY);
    ...
    Assembler::operKernel(stiffXX + stiffYY, mesh_, fe, AMat_);
...
void MixedFERegression<InputHandler,Integrator,ORDER>::smoothEllipticPDE()
{
	...
	typedef EOExpr<Real, StiffXX<Integrator, ORDER> > ETStiffXX;
	typedef EOExpr<Real, StiffYY<Integrator, ORDER> > ETStiffYY;
	typedef EOExpr<Real, StiffXY<Integrator, ORDER> > ETStiffXY;
	typedef EOExpr<Real, Grad<Integrator, ORDER> > ETGrad;

	Mass<Integrator, ORDER> EMass;
	StiffXX<Integrator, ORDER> EStiffXX;
	StiffYY<Integrator, ORDER> EStiffYY;
	StiffXY<Integrator, ORDER> EStiffXY;
	Grad<Integrator, ORDER> EGrad;

	ETMass mass(EMass);
	ETStiffXX stiffXX(EStiffXX);
	ETStiffYY stiffYY(EStiffYY);
	ETStiffXY stiffXY(EStiffXY);
	ETGrad grad(EGrad);
    ...
    Assembler::operKernel(c*mass+K(1,1)*stiffXX+K(1,2)*stiffXY+K(2,2)*stiffYY+dot(beta,grad), mesh_, fe, AMat_);
    Assembler::operKernel(mass, mesh_, fe, MMat_);
