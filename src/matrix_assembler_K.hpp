...
template <class Integrator, UInt ORDER, class Anysotropy>
class StiffAnys{
  private:
  ...
  public:
    ...
    StiffAnys(const Anysotropy& K): K_(K){};
...
class StiffAnys<Integrator, ORDER, Diffusivity>{
  private:
    //! A reference to FiniteElement<Integrator>
    /*!
     * Stores a reference to the finite element where the stiffness operator is evaluated.
...
class Stiff{
  ...
  public:
    ...
    inline StiffAnys<Integrator, ORDER, Diffusivity> operator() (const Diffusivity& K)
    {
    	StiffAnys<Integrator, ORDER, Diffusivity> anys(K);
    	return anys;
    }

    inline StiffAnys<Integrator, ORDER, Eigen::Matrix<Real,2,2> > operator() (const Eigen::Matrix<Real,2,2>& K)
    {
    	StiffAnys<Integrator, ORDER, Eigen::Matrix<Real,2,2> > anys(K);
       	return anys;
    }
};
...
class EOExpr{
     ...
     * returns a P variable.
     */      
	  EOExpr<P,A> operator() (const Eigen::Matrix<Real,2,2>& K)
      {
    	  return EOExpr<P,A>(A(K));
      }

	  EOExpr<P,A> operator() (const Diffusivity& K)
	  {
		  return EOExpr<P,A>(A(K));
	  }
